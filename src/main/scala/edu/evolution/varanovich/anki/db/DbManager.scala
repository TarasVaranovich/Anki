...
import edu.evolution.varanovich.anki.db.DbConfig.{dbDriver, dbPswd, dbUrl, dbUser}
...
object DbManager {
  def transactor(implicit shift: ContextShift[IO]): Resource[IO, Transactor[IO]] =
    for {
      ...
      blocker <- Blocker[IO]
      hikariTransactor <- HikariTransactor.newHikariTransactor[IO](dbDriver, dbUrl, dbUser, dbPswd, context, blocker)
